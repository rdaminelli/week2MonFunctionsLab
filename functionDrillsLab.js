...
function nameCheck(name){
  return name === 'Steve' ? 'What is up Steven' : name === 'Brian' ? 'Hey Bryan!' : 'Cool name, ' +name;
}
var nameGreeting = nameCheck('kitty')
...
function faveColorFind(color){
  return color === 'red' ? 'red is a great color' : color === 'green' ? 'green is a solid favorite color' : color === 'black' ? 'so trendy' : 'you need to evaluate your favorite color';
}
var colorRating = (faveColorFind('yellow'));
...
function bigOrSmall(arr){
  let answers =[];
  for (let i = 0; i < arr.length; i++){
    //return arr[i] > 100 ? answers.push('big') :answers.push('small');
    if (arr[i] > 100){
      answers.push('big');
    } else{
      answers.push('small');
...
function howManyChocolates(gold){
  return gold / 3;
}
totalFrogs = howManyChocolates(116);
...
function howManyChocolates2(gold){
  return `You can purchase ${Math.floor(gold/3)} chocolate frog(s) and your change is ${gold % 3} gold(s).`;
}
totalFrogs2 = howManyChocolates2(116);
